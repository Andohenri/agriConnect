...
const OrderCard = ({ order, userRole }: DirectOrderCardProps) => {
  const navigate = useNavigate();
  const statutConfig =
    ORDER_STATUT_CONFIG[order.statut || CommandeStatut.EN_ATTENTE];
  ...
  const isOuvert = order.statut === CommandeStatut.OUVERTE;
  const isFournie = order.statut === CommandeStatut.PARTIELLEMENT_FOURNIE;
  const isComplete = order.statut === CommandeStatut.COMPLETE;

  const isDemande = isOuvert || isFournie || isComplete;
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-start gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-start gap-2 mb-2">
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-bold line-clamp-1">
                    {produit?.nom || order.produitRecherche}
                  </h3>
                  {/* {produit?.sousType && (
              ...
              <Badge className={cn(statutConfig.color, "text-xs")}>
            ...
            >
              <DropdownItems
                ...
                style={"font-bold text-md bg-none"}
                title={"Voir details"}
              />
              <DropdownItems
                icon={<X color="red" />}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40">
                ...
                {/* Voir les détails - Toujours visible onViewDetails={() => navigate(`/orders/${order.id}`)} */}
                <DropdownMenuItem
                  onClick={() => navigate(`orders/${order.id}`)}
                >
                ...
                <DropdownMenuItem
                  ...
                  className="text-red-600 focus:text-red-600 focus:bg-red-50 flex items-center"
                >
                  <X size={16} color="red" />
                  <span>Annuler</span>
                </DropdownMenuItem>
                ...
                {userRole === Role.PAYSAN && (
                  <>
                    ...
                    {(isAccepted || isPaid || isDelivered) && (
                      <DropdownMenuItem
                      // onClick={() => onContact(order.collecteur?.id!)}
                      >
                ...
                {userRole === Role.COLLECTEUR && (
                  <>
                    ...
                    {(isAccepted || isPaid || isDelivered) &&
                      produit?.paysan?.id && (
                        <DropdownMenuItem
                        //   onClick={() => onContact(produit?.paysan?.id!)}
                        >
                          <MessageSquare size={16} className="mr-2" />
                          Contacter le paysan
                        </DropdownMenuItem>
        ...
        <div className="flex flex-col space-y-1.5 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-600">
              ...
              {formatPrice(produit?.prixUnitaire!)} Ar l'unité)
            </span>
          </div>
