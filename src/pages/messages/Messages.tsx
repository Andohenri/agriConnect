...
const Messages = () => {
  ...
  const messages = [
    { id: 1, sender: 'other', text: 'Bonjour, est-ce que le riz est toujours disponible?', time: '09:00' },
    { id: 2, sender: 'me', text: 'Oui, nous avons 500kg en stock', time: '09:05' },
    { id: 3, sender: 'other', text: 'Parfait! Je voudrais commander 100kg', time: '09:10' },
    { id: 4, sender: 'me', text: 'Excellente nouvelle! Je prépare votre commande', time: '09:12' },
    { id: 5, sender: 'other', text: 'Quand pouvons-nous récupérer?', time: '09:15' },
  ];
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden h-[calc(100vh-12rem)]">
      <div className="grid grid-cols-1 md:grid-cols-3 h-full">
        ...
        <div className={`${isMobile && selectedChat ? 'hidden' : 'block'} border-r border-gray-50`}>
          ...
          <div className="overflow-y-auto h-[calc(100%-120px)]">
            {chats.map(chat => (
              <div
                ...
                className={`p-4 border-b border-gray-200 hover:bg-gray-50 cursor-pointer transition ${selectedChat?.id === chat.id ? 'bg-green-50' : ''}`}
              >
        ...
        <div className={`${isMobile && !selectedChat ? 'hidden' : 'block'} md:col-span-2 flex flex-col`}>
          {selectedChat ? (
            <>
              <div className="p-4 border-b bg-gray-50 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-600">{selectedChat.online ? '● En ligne' : 'Hors ligne'}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                {messages.map(msg => (
                  <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs md:max-w-md ${msg.sender === 'me' ? 'bg-green-600 text-white' : 'bg-white'} rounded-2xl px-4 py-3 shadow`}>
