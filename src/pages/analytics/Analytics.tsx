      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-lg p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={salesData}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="ventes" stroke="#10b981" strokeWidth={3} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          ...
          <div className="space-y-4 mt-6">
            {[
              { label: 'Taux de conversion', value: '89%', color: 'bg-green-500', width: '89%' },
              { label: 'Satisfaction client', value: '94%', color: 'bg-blue-500', width: '94%' },
              { label: 'Livraisons à temps', value: '87%', color: 'bg-yellow-500', width: '87%' },
              { label: 'Taux de retour', value: '3%', color: 'bg-red-500', width: '3%' },
            ].map((item, i) => (
              <div key={i}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div className={`${item.color} h-3 rounded-full transition-all duration-1000`} style={{ width: item.width }}></div>
                </div>
              </div>
            ))}
          </div>
